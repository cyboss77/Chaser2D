/*    
  Chase spews out random colored pixels traveling at random speeds up
  and down a NeoPixel strip. There are a predetermined number of pixels
  traveling the strip; when one gets back to the beginning, a new one
  is regenerated. 
...
// hardware
#define PIN 4          // data output to the strip
#define NUM_LEDS 300   // how many pixels in the whole strip

// play with these numbers for different effects
#define NUM_CHASERS 20 // number of pixels on at any one time
#define MIN_DELAY 1    // min dwell time for a chaser
#define MAX_DELAY 12   // max dwell time for a chaser (MAX_DELAY - 1 : see random() )
...
int directions[ NUM_CHASERS ];           // +/-1 determines moves up or down the strip
...
{
  directions[n] = 1;    // move up the strip
